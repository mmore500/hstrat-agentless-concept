...
from pylib.auxlib._transform_neighborhood_max import transform_neighborhood_max


def test_transform_neighborhood_max_n0():
    ...
    result = transform_neighborhood_max(source_matrix, test_matrix, n=0)
    np.testing.assert_array_equal(result, expected)


def test_transform_neighborhood_max_n1():
    # Setup simple 3x3 matrices.
    source_matrix = np.array(
        [
            [10, 20, 30],
    ...
    result = transform_neighborhood_max(source_matrix, test_matrix, n=1)
    np.testing.assert_array_equal(result, expected)


def test_transform_neighborhood_max_n2():
    ...
    result = transform_neighborhood_max(source_matrix, test_matrix, n=2)
    np.testing.assert_array_equal(result, expected)


def test_transform_neighborhood_max_invalid_shape():
    # Test for mismatched shapes.
    source_matrix = np.array([[1, 2, 3]])
    test_matrix = np.array([[1, 2], [3, 4]])
    with pytest.raises(ValueError):
        transform_neighborhood_max(source_matrix, test_matrix, n=1)


def test_transform_neighborhood_max_not_2d():
    # Test when inputs are not 2D.
    source_matrix = np.array([1, 2, 3])
    test_matrix = np.array([1, 2, 3])
    with pytest.raises(ValueError):
        transform_neighborhood_max(source_matrix, test_matrix, n=2)
